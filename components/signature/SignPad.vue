<template>
  <div class="sign-pad">
    ...
    <div v-if="mode === 'draw'" class="relative">
      ...
      <div class="flex items-center justify-between mt-2">
        <div class="flex items-center gap-2">
          ...
          <input v-model="strokeColor" type="color" class="w-6 h-6 cursor-pointer" />
        </div>
        <button @click="clearDraw" class="text-sm text-gray-500 hover:text-gray-700">지우기</button>
      </div>
    </div>

    <!-- Text mode -->
    <div v-else-if="mode === 'text'" class="space-y-3">
      <select v-model="textFont" class="w-full px-3 py-2 border rounded-lg text-sm">
      ...
      <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 bg-white min-h-[80px] flex items-center justify-center">
        <input
          ...
          class="text-center text-3xl border-none outline-none bg-transparent w-full"
    ...
    <div v-else class="space-y-3">
      <div
        class="border-2 border-dashed border-gray-300 rounded-lg p-8 bg-white text-center min-h-[120px] flex flex-col items-center justify-center cursor-pointer hover:border-blue-400 transition"
    ...
    <div class="flex justify-end mt-4">
      <button
        ...
        class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-40 transition"
      >
        확인
      </button>
    </div>
...
function confirm() {
  ...
  if (mode.value === 'draw' && drawCanvas.value) {
    ...
    // Generate text as canvas image for consistent rendering
